...
element的type类别还有“ray”（射线）、“segment”（线段）、“boolean”、“button”、“list”等等，而能进行gmt转换的只有前2种ray、segment以及示例中的3种point、line、conic。
...
以下仅填写name、input，output默认输出一个，会输出多个时会表明。（以下函数在readme中已有介绍，故从简）
 * Ray：射线，a0为端点，a1为经过的点
 * Segment：线段，a0为第一个端点，a1为第二个端点
 * Circle：圆，
    * （三点圆）如果有a2，就是经过3点a0、a1、a2的圆；
    * （圆规）如果a1是线段或者长度（包括线段对象、“Segment[A, B]”及“Radius[c]”），则是以a0为圆心，a1为半径的圆；
    * （圆）如果a1是点，则是以a0为圆心，a1为圆上一点的圆
 * Point：对象上的点，a0为对象
 * Intersect：交点，a0为第一个对象，a1为第二个对象，a2控制是哪个交点（线线交点没有a2）
 * Midpoint：中点，
    * （两点）如果有a1，则a0为第一点，a1为第二点；
    * （线段）如果只有a0，则为线段中点
 * OrthogonalLine：垂线，a0为垂线上一点，a1为与其垂直的线
 * LineBisector：中垂线，
    * （两点）如果有a1，则a0为第一点，a1为第二点；
    * （线段）如果只有a0（线段），则为线段中垂线
 * AngularBisetctor：角平分线，
    * （三点）a0、a2分别为角两边上一点，a1为角的顶点；
    * （两线）如果没有a2而是两线a0、a1，**输出其两条角平分线**
 * Center：中心，a0为一圆
 * Tangent：切线，
    * （点）a0为一点，a1为一圆，**输出通常有两个**，如果点在圆上那么输出只有一个；
    * （直线）如果a0为一线，则为圆的与其平行的两切线，**输出通常有两个**
 * Polar：极线，a0为一点，a1为一圆
 * Diameter：径线，a0为一线，a1为一圆，生成 过圆a1的圆心 且垂直于a0 的垂线

以下为ggb中使用的自定义工具
 * PolarPoint：极点，a0为一线，a1为一圆
 * EdgePoint：无穷远点，a0为一线，**输出两**假想的点
...
* **预绘制：**
  ...
  * Intersect[对象1,对象2,x,A?] - 两个对象的一个交点，
    * 关于x，两线交点x=0，线与圆交点、圆与圆交点x=0或1（若是线与圆，则按*线的方向*计数；若是圆与圆，则以连心线的绘制方向起逆时针计数），
        * 线的方向情况很复杂，如直线的方向：第一定义点至第二定义点的方向；
        * 垂线方向：参考线方向顺转90°；
        * 中垂线方向：参考线方向顺转90°；
        * 还有角平分线、切线等等，建议自己尝试；
    * 第四个参数猜测是作排除用，表示两对象非A点的另一个交点（一般用在移动对象时可能引起的参数x发生颠倒导致两点重叠等错误时），如果写了，参数x可能会被无视；不写不会引起歧义，可以省略或填写“-”；
  * Linepoint[对象,x] - 对象上一点，
    * 对象是圆，x为（x轴正方向起）顺时针弧长。
    * 对象是线的情况很复杂，如直线是第一个点正方向与两定义点间距离倍数；
        * 垂线是定义点“正方向”与被垂直的线定义点间距离倍数；
        * 中垂线虽然也是两定义点间正方向距离倍数，但是正方向起点在45°夹角处；
        * 角平分线则是顶点正方向与坐标系100单位长度倍数；
        * 切线是切点为起点正方向与切圆半径的倍数，
        * 还有一些，建议自己尝试；
  * Midpoint[A,B] - A、B的中点；
  * Perp[A,s1] - 过A作s1的垂线；
  ...
  * Tangent[A,c1,x] - 过A作c1的切线，x=0或1，从0到1依次为两个切点的顺序，从圆心到点逆时针数，如下图；
  <img src="icon/p2.png" width = 50% align=center />
  * Circle3[A,B,C] - 过A、B、C三点作圆；
  * CopyAngle[A,B,C,D,E] - 以E为新顶点，D为新边上一点，复制角ABC，做出射线（需要注意方向，如下图）；
  <img src="icon/p1.png" width = 50% align=center />
...
* **设定：**
  ...
  * rules - 题目限制（限制题目给出图形的位置关系，就是有些题移动对象时会出现的红色虚线），如
    * “对象1#对象2”表示两者必须相交，
    * “对象1/对象2”表示两者不能相交，
    ...
    * 有必要写rules的情况：
        ...
        * （均衡）移动给出图形后，某一情况下不能用同一种（最优）解法作出所有的解
        * （跳变）移动给出图形后，某一情况下result判定不出来的，或是result判定的解与题目描述的解不符
